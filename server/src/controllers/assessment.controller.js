...
class AssessmentController {
  ...
  async getAssessments(req, res) {
    try {
      ...
      const assessmentsWithStatus = assessments.map(assessment => ({
        ...assessment.toObject(),
        completed: userResults.includes(assessment._id),
  ...
  async startAssessment(req, res) {
    try {
      ...
      const questions = await assessmentService.generatePersonalizedQuestions(assessment, user);
  ...
  async submitAssessment(req, res) {
    try {
      ...
      // Calculate scores
      const scores = await assessmentService.calculateScores(assessmentId, responses);
